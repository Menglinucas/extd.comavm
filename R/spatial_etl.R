...
getpoi.ras<-function(month_offset = -2,resto=200,
                     con=NULL,
                     tabln.vec=NULL
                     ){
    library(dplyr)
    ...
    poi.dens.pts<-suppressWarnings(sapply(poi.dens,raster::extract,y=ha_info.sp,sp=F))
    ...
    poi.diversity.pts<-suppressWarnings(raster::extract(x = poi.diversity,y=ha_info.sp,sp=F))
